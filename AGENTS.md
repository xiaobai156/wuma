...
失败 TXT 重抓入口为 `python crawler.py --retry-failed --issues 期数`。自动读取对应单期失败 TXT，只抓取其中按名称、URL 和方向精确匹配当前启用配置的站点，重复失败行只抓一次；文件缺失、损坏、包含其他期或未知/停用站点时停止，绝不回退全站。省略期数时仅在输出目录恰有一份标准单期失败 TXT 时自动选择，多份时必须指定期数，不按修改时间猜期，不选择多期文件。
失败重抓逐项保存成功结果：仅追加本次成功站点、仅清理该站同一期失败行，其余失败记录及原成功 TXT 的字节、BOM、换行和顺序保持不变。没有成功结果或失败文件为空时不修改正式 TXT 和缓存。抓取期间失败 TXT 变化时停止提交，防止覆盖另一轮结果；TXT 写入异常沿用事务回滚。
失败重抓和指定目录修复共用定向缓存更新，只合并成功站点的真实结果、清除其同一期失败标记；其他站点（含未启用站点）的对象、身份、历史、扩展字段及失败标记保持不变。缓存冲突/格式错误时停止缓存同步，已定稿 TXT 保留；需要补同步时可重新执行指定目录修复入口，不扩大到全站。仍有失败或缓存更新未完成时失败重抓返回非零状态。
无网络回归命令为 `python -m unittest discover -s tests -v`；测试使用临时目录和隔离页面响应，不得改动正式 TXT、targets.json 或 recent_10_cache.json。
